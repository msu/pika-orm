...
```
*Note: This does not solve the N+1 problem for the initial load, but it prevents it from becoming 2N+1 or worse.* See the [Query Caching Pattern]({{ site.baseurl }}/docs_draft/Patterns/Feature%20Patterns/query-caching-pattern.md) for more details.
