...
description: "Complete graph-based map of every PikaORM class, interface, and subsystem with Mermaid diagrams."
...
`PikaORM` is the **single entry point** for all database operations. It is never sub-classed; instead it is configured at startup with a builder pattern.
...
sequenceDiagram
    ...
    DB-->>B: 0 rows affected X → update() returns false
```
